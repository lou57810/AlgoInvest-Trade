...
def recursif_algo(max_price, data, selected_actions):      
    if data:                
        ...
        if float(action.price) <= max_price:                        
            action2, list_actions2 = recursif_algo(max_price - float(action.price), data[1:], selected_actions + [action])
        ...
        return sum([float(i.benefit) for i in selected_actions]), selected_actions
