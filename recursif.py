from action import Action
import csv
from algo import Algo


class Recursiv(Algo):


    def __init__(self, file, max_price):
        super().__init__(file, max_price)        
        self.selected_actions = []
    
    def algo_generic(self):
        data = super().get_datas(self.file)          # tableau des actions avec prix et benefices non triés
        recursif_algo(self.max_price, data, self.selected_actions)
        super().display_algo_data(tab[-1])

tab = []
def recursif_algo( max_price, data, selected_actions):      # 10 mn / 12 actions    
    
    if data:                
        action1, list_actions1 = recursif_algo(max_price, data[1:], selected_actions)        
        action = data[0]

        if int(action.price) <= max_price:                        
            action2, list_actions2 = recursif_algo(max_price - int(action.price), data[1:], selected_actions + [action])
            
            if int(action1) < int(action2):
                return int(action2), list_actions2
        tab.append(list_actions1)        
        return int(action1), list_actions1
        
    else:
        #tab.append(selected_actions)
        #tab = sum([i.benefit * 100 for i in selected_actions]), selected_actions
        #print('tabl', tab)
        return sum([i.benefit for i in selected_actions]), selected_actions
